...
FLAGS, FLAGS_DEF = define_flags_with_default(
    ...
    decode_lengths_min=1000,
    decode_lengths_max=32000,
    n_decode_length_intervals=3,
    n_rounds=3,
    seed=1234,
    mesh_dim='1,-1,1,1',
    dtype='fp32',
...
class LLMGenTester:
    ...
    def __init__(self,
                 decode_lengths_min = 1000,
                 decode_lengths_max = 126000,
                 decode_lengths_num_intervals = 10,
                 n_rounds = 3,
        ...
        self.model = Sampler()
        self.context_lengths = [self.model.block_size * 8]
    ...
    def print_start_test_summary(self):
        print ("\n")
        print ("Starting Gen Testing...")
        print (f"- Decode Lengths: {len(self.decode_lengths)}, Min: {min(self.decode_lengths)}, Max: {max(self.decode_lengths)}")
        print ("\n\n")
...
class Sampler:
    def __init__(self, min_new_tokens=1024):
    ...
    def min_new_tokens(self, value):
        self._min_new_tokens = (value + self.block_size) // self.block_size * self.block_size
    
    @min_new_tokens.getter
    def min_new_tokens(self):
        return self._min_new_tokens
...
def main(argv):
    ...
    ht = LLMGenTester(
        decode_lengths_min=FLAGS.decode_lengths_min,
        decode_lengths_max=FLAGS.decode_lengths_max,
        decode_lengths_num_intervals=FLAGS.n_decode_length_intervals,
